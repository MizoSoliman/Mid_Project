...
if page == 'Introduction':
    ...
    st.image('Article_1_1.jpg', width=1200)
    st.write("""
    ...
    st.write("- **Total Time** :")
    data['total_time'] = data['registration_time'] + data['nursing_time'] + data['laboratory_time'] + data['consultation_time'] + data['pharmacy_time']
    st.code("""
    data['total_time'] = data['registration_time'] + data['nursing_time'] + data['laboratory_time'] + data['consultation_time'] + data['pharmacy_time']
            """)
    
    st.write("- **Age Grouping** :")
    data['age_group'] = pd.cut(data['age'] , bins =[-0.1, 3, 17, 35, 50, 100], labels=['infant', 'child', 'young', 'adult', 'senior'])
    st.code("""
            data['age_group'] = pd.cut(data['age'] , bins =[-0.1, 3, 17, 35, 50, 100], labels=['infant', 'child', 'young', 'adult', 'senior'])
            """)
    ...
    st.write("- **Visit Day Type** :")
    data['visit_day_type'] = data['dayOfWeek'].apply(lambda x: 'Weekend' if x in [5, 6] else 'Weekday')
    st.code("""
            data['visit_day_type'] = data['dayOfWeek'].apply(lambda x: 'Weekend' if x in [5, 6] else 'Weekday')
            """)
    ...
    """)
    most_common_day = data.day.value_counts().sort_values(ascending=False).idxmax()
    ...
    data.columns = data.columns.str.strip().str.replace(' ', '_').str.lower()
    data.visit_date = pd.to_datetime(data.visit_date)
    data['day'] = (data.visit_date.dt.day).astype(int)
    data['month'] = (data.visit_date.dt.month).astype(int)
    data['year'] = (data.visit_date.dt.year).astype(int)
    data['dayOfWeek'] = data['visit_date'].dt.dayofweek
    data.drop(['visit_date'], axis=1, inplace=True)
    ...
    for col in categoric_columns:
        st.write(f"Column: {col}")
        st.write(f"Unique values: {x_train[col].nunique()}")
        st.markdown("---") 
    st.markdown(
        """
    ...
    one_df_train = pd.DataFrame(one.fit_transform(x_train[['gender' , 'has_insurance' , 'age_group' , 'visit_day_type']]) , columns=one.get_feature_names_out())
    one_df_test = pd.DataFrame(one.transform(x_test[['gender' , 'has_insurance' , 'age_group' , 'visit_day_type']]) ,  columns=one.get_feature_names_out())
    st.code(
        """
        one = OneHotEncoder(sparse_output=False, drop='first')
        one_df_train = pd.DataFrame(one.fit_transform(x_train[['gender' , 'has_insurance' , 'age_group' , 'visit_day_type']]) , columns=one.get_feature_names_out())
    ...
    st.markdown(
        ...
        """)
    x_train = pd.concat([x_train, one_df_train, bie_df_train], axis=1).drop(columns=['gender' , 'has_insurance' , 'area' , 'diagnosis', 'visit_day_type' , 'age_group'] , axis=1)
    st.code(
            """
            x_train = pd.concat([x_train, one_df_train, bie_df_train], axis=1).drop(columns=['gender' , 'has_insurance' , 'area' , 'diagnosis', 'visit_day_type' , 'age_group'] , axis=1) 
            x_train
    ...
    st.markdown('<h1 class="fade-in" style="text-align:center; color:lightblue;">Conclusion</h1>', unsafe_allow_html=True)
    ...
    <div class="social-icons fade-in">
        <a href="https://www.facebook.com/share/19Vr7gwf6z/" target="_blank">
            <img src="https://cdn-icons-png.flaticon.com/512/124/124010.png" alt="Facebook">
        </a>
        <a href="https://wa.me/201149073567" target="_blank">
            <img src="https://cdn-icons-png.flaticon.com/512/733/733585.png" alt="WhatsApp">
        </a>
        <a href="https://github.com/MizoSoliman" target="_blank">
            <img src="https://cdn-icons-png.flaticon.com/512/733/733553.png" alt="GitHub">
        </a>
        <a href="mailto:mazinyaser0@gmail.com" target="_blank">
